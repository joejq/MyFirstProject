            ...
            switch (message.Type)
            {
                ...
                case WeixinMessageType.Image://图片消息
                    ...
                    try
                    {
                        ...
                        //meg = await OCRHelper.MakeRequest(message.Body.PicUrl.Value);
                    }
                    catch (Exception ex)
                    {
                        meg = ex.Message;
                ...
                case WeixinMessageType.Voice://语音消息
                    #region 语音消息
                    {
                        ...
                        var msgId = message.Body.MsgId.Value.ToString();
                        //TODO
                        result = ReplayPassiveMessageAPI.RepayText(openId, myUserName, string.Format("语音消息:openid:{0},media_id:{1},format:{2},msgId:{3}", format, media_id, format, msgId));
                    }
                    #endregion
                    break;
